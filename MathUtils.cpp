...
float squishify(float t_value, float k) 
{
	return (std::log(1.0f+std::exp(k*t_value))-std::log(1.0f+std::exp(k*(t_value-1.0f)))) / k;
};


float quantize(float t_value, float t_stepCount)
{
	return (std::floor(t_value * t_stepCount) + 1.0f) / t_stepCount;
};
